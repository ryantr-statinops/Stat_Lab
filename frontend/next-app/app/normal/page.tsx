...
export default function NormalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.get<NormalResponse>(
        `${API_URL}/api/v1/normal`,
        { params }
      );
      setResult(res.data);

      // Binning phía client bằng helper dùng chung — biểu đồ 40 bins
      const bins = histogramBins(res.data.samples, 40);
    ...
    <main className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <header>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Z = √(−2 ln U₁) · cos(2πU₂) — biến đổi hai biến đều thành hai chuẩn
          tắc độc lập.
        </p>
      </header>
