...
export default function CltPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/v1/clt?${qs}`);
      if (!res.ok) {
      ...
      const data = (await res.json()) as CltResponse;
      setResult(data);

      // Phân phối của trung bình mẫu — binning client-side
      const bins = histogramBins(data.sample_means, 40);
  ...
  return (
    <main className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <header>
        <p className="text-sm font-medium text-blue-600">Central Limit Theorem</p>
        <h1 className="text-2xl font-bold mt-1">Định lý Giới hạn Trung tâm</h1>
        ...
        <section className="space-y-4">
          <div className="flex flex-wrap gap-2 text-xs">
            ...
            <span className="px-2.5 py-1 rounded-full bg-gray-100 text-gray-700">
              x̄ = {result.empirical_mean.toFixed(4)} ≈ μ = {result.theoretical_mean}
            </span>
            <span
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 className="font-semibold mb-4">Phân phối của trung bình mẫu</h2>
            ...
            <p className="text-xs text-gray-400 mt-3">
              Đường đỏ đánh dấu kỳ vọng lý thuyết μ. Tăng kích thước mẫu n để xem
              histogram thu hẹp quanh μ với độ rộng ≈ SE = σ/√n.
            </p>
          </div>
        </section>
      )}
