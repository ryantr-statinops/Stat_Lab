...
export default function LcgPage() {
  ...
  // Lưu chuỗi thô để ô input được rỗng khi xoá (pattern chuẩn đã fix ở next-app)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/v1/lcg?${qs}`);
      if (!res.ok) {
      ...
      setResult((await res.json()) as LcgResult);
    } catch (err) {
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                fullPeriod ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"
              }`}
            >
